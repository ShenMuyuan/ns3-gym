...
int main (int argc, char *argv[])
{
  ...
  uint64_t accum = 0;
  std::for_each (std::begin(cpp2py_durations), std::end(cpp2py_durations), [&](const double d) {
      accum += (d - cpp2py_cycle_mean) * (d - cpp2py_cycle_mean);
  });
  auto cpp2py_cycle_stddev = sqrt(accum / (cpp2py_durations.size() - 1));
  accum = 0;
  std::for_each (std::begin(py2cpp_durations), std::end(py2cpp_durations), [&](const double d) {
